...
public class MonsterWalk : BaseMonster
{
	protected Coord coord;
	protected Coord nextStep = null;
	...
	bool findNextStep(int x,int y,int dx,int dy){
		...
		if(Utils.IsCoordInMap(x+dx,y+dy)){
			var s = map.pathMapWalk[map.IndexByCoord(x,y)];
			var s2 = map.pathMapWalk[map.IndexByCoord(x+dx,y+dy)];
			if(s2>=0 && (s2 == s-1 || s == 999) && map.buildMap[map.IndexByCoord(x+dx,y+dy)] != 1 && map.crowdedMapGround[map.IndexByCoord(x + dx, y + dy)] < Configs.commonConfig.crowdedMaxInCoord){
				return true;
			}
		}
		return false;
	...
	bool findNextStepDiagonal(int x,int y,int dx,int dy){
		...
		if(Utils.IsCoordInMap(x+dx,y+dy)){
			var s = map.pathMapWalk[map.IndexByCoord(x,y)];
			var s2 = map.pathMapWalk[map.IndexByCoord(x+dx,y)];
			var s3 = map.pathMapWalk[map.IndexByCoord(x,y+dy)];
			var s4 = map.pathMapWalk[map.IndexByCoord(x+dx,y+dy)];
